...
LinkedMap.prototype = {
	 ...
	 * 
	 * @param callback {function(key:string, value:object):boolean}
	 *  - called for every entry in context of current LinkedMap. 
	...
	each : function(callback, startIndexset) {
		...
		var i = (typeof(startIndex) === 'number' && startIndex > 0) ? startIndex : 0;
		for(; i < this._list.length; ++i) {
			if(callback.call(this, this._list[i], this._map[this._list[i]].value) === false ) {
				return null; //brake
			}
		}
		return null;
	...
	move : function(key, targetKey) {
		...
		var key = this._list.splice(index, 1)[0];//remove key
		var targetIndex = this._list.indexOf(targetKey);//if key not found, we get -1, that is safe for splice
		this._list.splice(targetIndex, 0, key);
	},
